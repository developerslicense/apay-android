...
public final class DataEntryActivity extends Activity implements TextWatcher {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        capture = getIntent().getParcelableExtra(NewCardIOActivity.EXTRA_SCAN_RESULT);
        ...
        if (capture != null) {
            ...
            cardView.setImageBitmap(NewCardIOActivity.markedCardImage);
        ...
        boolean requireExpiry = getIntent().getBooleanExtra(NewCardIOActivity.EXTRA_REQUIRE_EXPIRY, false);
        boolean requireCVV = getIntent().getBooleanExtra(NewCardIOActivity.EXTRA_REQUIRE_CVV, false);
        boolean requirePostalCode = getIntent().getBooleanExtra(NewCardIOActivity.EXTRA_REQUIRE_POSTAL_CODE, false);
        ...
        if (requirePostalCode) {
            ...
            boolean postalCodeNumericOnly =
                    getIntent().getBooleanExtra(NewCardIOActivity.EXTRA_RESTRICT_POSTAL_CODE_TO_NUMERIC_ONLY, false);
    ...
    private void completed() {
        ...
        dataIntent.putExtra(NewCardIOActivity.EXTRA_SCAN_RESULT, result);
        if(getIntent().hasExtra(NewCardIOActivity.EXTRA_CAPTURED_CARD_IMAGE)){
            dataIntent.putExtra(NewCardIOActivity.EXTRA_CAPTURED_CARD_IMAGE,
                    getIntent().getByteArrayExtra(NewCardIOActivity.EXTRA_CAPTURED_CARD_IMAGE));
        }
        DataEntryActivity.this.setResult(NewCardIOActivity.RESULT_CARD_INFO, dataIntent);
        finish();
    }

    @Override
    public void onBackPressed() {
        DataEntryActivity.this.setResult(NewCardIOActivity.RESULT_ENTRY_CANCELED);
        finish();
    }
    ...
    private void addCardholderNameIfNeeded(ViewGroup mainLayout) {
        boolean requireCardholderName = getIntent().getBooleanExtra(NewCardIOActivity.EXTRA_REQUIRE_CARDHOLDER_NAME, false);
